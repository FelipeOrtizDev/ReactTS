...
const ServicosEmAndamentoPage: React.FC = () => {
    ...
    const [selectedSolicitacao, setSelectedSolicitacao] = useState<SolicitacaoBase | null>(null);
        ...
        const fetchSolicitacoes = async () => {
          try {
            ...
            const initialTimeLeft = data.reduce((acc, solicitacao) => {
              const previsao = solicitacao.SB_Acatamento?.SB_PrevisaoAcatamento || "00:00:00";
              const previsaoDate = new Date(`1970-01-01T${previsao}Z`).getTime();
              const now = Date.now();
              const timeDifference = previsaoDate - now;
              acc[solicitacao.id_SolicitacaoBase] = timeDifference > 0 ? timeDifference : 0;
              return acc;
    ...
    const handleEditClick = async (solicitacao: SolicitacaoBase) => {
        ...
        try {
            // Carregar Fechamento com base no ID de SolicitacaoBase
            const testeFechamento = await getFechamentos(solicitacao.id_SolicitacaoBase);
            console.log('LOAD Fechamento carregado:', testeFechamento);
            
            // Atualizar o Zustand com os dados de Fechamento usando setFechamento
            const setFechamento = useStore.getState().setFechamento;
            setFechamento(testeFechamento[0]); // Define o primeiro fechamento ou um objeto vazio
    
            // Atualizar o Zustand com os dados de SolicitacaoBase
            setSolicitacaoBase(solicitacao);
            
            // Atualizar o estado local para armazenar a solicitação selecionada
            setSelectedSolicitacao(solicitacao);
    
            // Verificar se o estado foi atualizado corretamente
            console.log('Fechamento atualizado no Zustand:', useStore.getState().fechamento);
                ...
                <ListContainer>
                    ...
                    <BodyTable>
                        {solicitacoes.map((solicitacao) => {
                            const timeLeftForSolicitacao = timeLeft[solicitacao.id_SolicitacaoBase];
